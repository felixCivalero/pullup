-- 050_remove_campaigns.sql
-- Remove the email-campaign feature entirely. PullUp no longer sends styled /
-- templated mass email from the platform (host campaigns OR the admin
-- platform newsletter) — relationships are kept feeling like relationships,
-- and bulk sends are plain, personal, important-info only. All campaign code
-- (endpoints, MCP tools, services, composer UI) has been removed; this drops
-- the now-orphaned tables and the campaign-analytics function.
--
-- Safe to run after the code deploy: nothing writes these tables anymore, and
-- transactional email tracking (email_outbox / email_opens / email_clicks /
-- email_suppressions) is untouched. email_outbox.campaign_send_id is left in
-- place as a nullable column once its FK is dropped with the table.
--
-- Idempotent: guarded with IF EXISTS / CREATE OR REPLACE.

begin;

-- 1) Rewrite anonymize_person() (latest def: migration 042) WITHOUT the
--    campaign_sends scrub line — that table is being dropped below. Every
--    other scrub is preserved verbatim so right-to-erasure still works.
...
-- 2) Drop the campaign-analytics aggregation function (host_email_summary).
--    It read campaign_sends joined to email_outbox; the "how are my emails
--    doing" surface is gone with campaigns.
drop function if exists public.host_email_summary(uuid, int);

-- 3) Drop the campaign tables. CASCADE removes email_outbox.campaign_send_id's
--    FK constraint (the nullable column itself stays — harmless dead column).
drop table if exists campaign_sends cascade;
drop table if exists campaign_campaigns cascade;
