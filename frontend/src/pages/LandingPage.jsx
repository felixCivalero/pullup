...
// Raw-import the PullUp eye SVGs so we can render them inline and
// directly mutate the pupil paths' transforms when the cursor moves.
...
/* ─── Cursor-tracking PullUp eyes ───
   Renders the brand "eyes" SVG inline (via Vite's ?raw import) and
   mutates the pupil paths' `transform` attributes on mousemove so the
   pupils track the cursor — each eye tracks from its own center, so
   they naturally diverge or converge.

   We identified the pupil paths by inspecting the original SVGs: their
   path elements carry `transform="translate(x, y)"` attributes, and
   the pupils are the paths whose (x, y) sit near where the eyes are
   in the 2761×2418 canvas. For bigeyes there's an extra highlight
   path that moves with the right pupil so the sparkle stays attached. */
const EYE_VARIANTS = {
  small: {
    svg: smalleyesSvg,
    movables: [
...
// Both source SVGs declare width/height but no viewBox, so when CSS
// shrinks them to ~50px tall the paths still draw at native 2761-coord
// scale and get clipped out of view. Inject a viewBox so the path
// coordinate space scales to fit the rendered box.
function injectViewBox(rawSvg) {
  if (/\sviewBox=/i.test(rawSvg)) return rawSvg;
...
function PullupEyes({ variant = "small", className }) {
  ...
  useEffect(() => {
    ...
    // Find the pupil/highlight paths by matching their original
    // translate(...) against the known coords for this variant.
    // We stash the originals on each node via data-* attributes so we
    // can always recover them even if our transform has been mutated.
    const findTargets = () => {
      const found = [];
    ...
    const apply = () => {
      ...
      // Self-heal: if any cached path got detached (rare, e.g. an
      // unrelated re-mount), re-query from the wrap.
      if (targets.length === 0 || !targets[0].node.isConnected) {
        targets = findTargets();
        if (targets.length === 0) return;
      }
      // Rect is recomputed every frame because the slide shell uses CSS
      // transforms that don't fire scroll/resize events.
    ...
    // pointermove covers mouse + pen + touch and is more reliable than
    // mousemove on hybrid devices.
